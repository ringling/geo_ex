...
defmodule WKB do
  ...
  defp reader_read(count, reader) do
    ...
    if reader.endian == :ndr do
      value = Utils.reverse_byte_order(value)
    end
  ...
  def do_encode(geom, writer) when is_list(geom) do
    ...
    srid = ""
    if hd(geom).srid do
      srid = Integer.to_string(hd(geom).srid, 16) |> Utils.pad_left(8)
    end

    count = Integer.to_string(Enum.count(geom), 16) |> Utils.pad_left(8)
  ...
  defp encode_coordinates(:point, coordinates, writer) do
    if coordinates == [0,0] do
      writer_write(Utils.repeat("0", 32), writer)
    else
      x = coordinates |> hd |> Utils.float_to_hex(64) |> Integer.to_string(16)
      y = coordinates |> List.last |> Utils.float_to_hex(64) |> Integer.to_string(16)
  ...
  defp encode_coordinates(type, coordinates, writer) when type == :multi_point or type == :multi_line_string or type == :multi_polygon do
    ...
    writer = writer_write(Integer.to_string(length(coordinates), 16) |> Utils.pad_left(8), writer)
